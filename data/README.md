...
| `data/cik_review.csv` | committed: borderline name matches, one row each, for a person to mark `confirmed` yes or no; decided rows are kept unchanged across reruns and only new pairs are added; only names that can change a contract's company are listed, and close spellings accepted automatically are listed as `auto` so they can be rejected with `no` |
